...
export function ConfigProvider({ children }) {
  ...
  const applyThemeColors = (theme) => {
    ...
    if (theme.primaryColor) {
      ...
      root.style.setProperty("--color-surface-tint", theme.primaryColor);
    }
    if (theme.primaryContainer) {
    ...
    if (theme.textColor) {
      root.style.setProperty("--color-on-background", theme.textColor);
    }
  };
  ...
  const updateConfig = async (newConfig) => {
    try {
      ...
      if (res.ok) {
        ...
        applyThemeColors(data.config.theme);
